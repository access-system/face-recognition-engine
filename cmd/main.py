...
def main():
    ...
    video_stream = VideoStream(stop_event, lock, shared_frames, shared_access, log, fps=fps)
    ...
    recognition_arcface = RecognitionArcFace(stop_event, lock, face, shared_embedding, log, device='GPU',
                                             fps=fps)
    embedding_validation = EmbeddingValidation(stop_event, cache, lock, shared_embedding, shared_access, log, fps=fps)
